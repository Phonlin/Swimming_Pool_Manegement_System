...
//       顯示卡片內容
      ...
      Serial.println();
      Serial.print(F("CC:"));
      Serial.println(mfrc522ReadCode);
//        mfrc522ReadCode = "";
  ...
  if (irValue > FINGER_ON ) {
    ...
    if (beatAvg > 65)  {
      Serial.println(",SPO2:" + String(ESpO2));
        String INSERT_SQL = "INSERT INTO swim.people (idrfid,heartbeat,blood_oxygen) VALUES ('" + String(String(mfrc522ReadCode)) + "','" + String((int)beatAvg) + "','" + String((int)ESpO2) + "')";
...
void dump_byte_array(byte *buffer, byte bufferSize) {
  ...
  for (byte i = 0; i < bufferSize; i++) {
    ...
    mfrc522ReadCode += String(buffer[i] < 0x10 ? " 0" : " ");
    mfrc522ReadCode += String(buffer[i], HEX);  
  mfrc522ReadCode.toUpperCase();
  k=1;
  }
  }
    delay(2000);
